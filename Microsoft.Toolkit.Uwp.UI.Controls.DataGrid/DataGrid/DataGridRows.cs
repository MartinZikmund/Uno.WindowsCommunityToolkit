...
namespace Microsoft.Toolkit.Uwp.UI.Controls
{
    ...
    public partial class DataGrid
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("-----------------------------------------------RowGroupHeaders");
            foreach (int slot in this.RowGroupHeadersTable.GetIndexes())
            {
                DataGridRowGroupInfo info = this.RowGroupHeadersTable.GetValueAt(slot);
#if FEATURE_ICOLLECTIONVIEW_GROUP
                System.Diagnostics.Debug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0} {1} Slot:{2} Last:{3} Level:{4}", info.CollectionViewGroup.Name, info.Visibility.ToString(), slot, info.LastSubItemSlot, info.Level));
#else
                System.Diagnostics.Debug.WriteLine(string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0} Slot:{1} Last:{2} Level:{3}", info.Visibility.ToString(), slot, info.LastSubItemSlot, info.Level));
#endif
            }

            System.Diagnostics.Debug.WriteLine("-----------------------------------------------CollapsedSlots");
